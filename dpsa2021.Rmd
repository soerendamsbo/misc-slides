...
**RQ: Does bad weather on Election Day cause voters to stay home and not turn out for election?**

--

> "*It's __a myth that the weather matters for turnout__. If the weather is bad, maybe a few voters stay home. But today's weather is probably of little importance.*".right[Jørgen Elklit, Ritzau, 15 Nov 2005]

--

> "*As it seems, one can expect that the effect of the weather at Danish parliamentary Elections is lower than in US presidential elections, as the political interest is greater. It simply __takes more to make voters stay home__.*".right[Yosef Bhatti, Videnskab.dk, 31 Jan 2011] 

--

> "*There are lots of things that affect turnout, but the weather does not seem to be one of them. In the Danish context, we have tried and tried to show that when it rains, people will stay at home. But __the Danes turn out no matter if it rains__ or snows*".right[Kasper M. Hansen, DR Nyheder, 18 Jun 2015]
...
- We show that existing research (30 studies) is highly mixed in terms of design and results

--

- We provide better evidence to the matter through several methodological advantages:
...
- Rainfall ("bad weather") decreases probability of voting (1.1 pp per cm rain) 
...
- The rainfall effect grows as rainfall levels increase (is non-linear)
...
- Meta-analysis reveals mixed results and heterogeneous designs

--

- Average effect of rainfall: -0.76 percentage points per cm of rain
...
- Marginal voters are "*those whose decisions to turn out are sensitive to exogenous factors*" (Fowler 2015: 205)
...
- Validated voter records for all* eligible Danish voters at municipal elections of 2013 and 2017

.footnote[*Excluding ~1% and ~9% in non-participating municipalities, ~3% early voting, and elderly voters above age 80.]

--

- 4,459,145 eligible in-person voters (age 18-80) &rarr; ~73% eligible at both elections

--

- Home coordinates enables us to calculate individual weather exposure based on validated DMI data

---

# Weather observations from 166 DMI weather stations
...
_Example_
```{r stationsexample}
include_graphics("media/weather-stations-example.png")
```
]

.pull-right[

- Accumulated rainfall, avg. sunshine, and avg. temperature aggregated for each weather station from hourly values on Election Day

- Individual weather exposure: Inverse distance weighted average from three nearest weather stations

- $Local\ weather_v = \frac{\sum_{i}^{3} (\frac{Observation_i}{Distance_i})}{\sum_{i}^{3} (\frac{1}{Distance_i})}$
...
- OLS regression of in-person turnout (1/0) on Election Day rainfall, sunshine, and temperature
...
- Additional controls: (individual) age, gender, (municipal) population size, share of non-Western immigrants, and closeness of the election
...
```
<span style="color:grey;font-size:8pt;">Edward Linsmier—Getty Images 2014, time.com/3554884/2014-election-weather-forecast</span>
